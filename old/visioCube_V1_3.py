...
DBP = ihmCube.DBP
NBPCLN = 7
TEXTBP = ['CAMÉRA 1', 'CAMÉRA 2', 'CAMÉRA 3', 'CAMÉRA 4', 'RECADRAGE', 'RESET POINT', 'EFFACER POINT', 'MESURER', 'RESET MESURES', 'RECONNAITRE', 'VÉROUILLER#DÉVÉROUILLER', 'ECLAIRER +', 'ECLAIRER -', 'QUITTER']
print(XBP, XBP2)
...
XBP3 = 10
DBP3 = 10
TEXTBP3 = [c + m for c in 'FRUBLD' for m in ("", "'", "2")]
...
class Reconnaissance(object):
    def __init__(self, n, cube, robot=None):
        ...
        self.enable_modif = False
        self.bp_not_enable = (4, 5, 6, 7, 8, 11, 12)
        self.bp_verouillage = 10
    ...
    def _affiche_nb_mesure(self):
        txt = ""
        for f in self.visio.cameras[self.numCam].dicoCouleur:
            txt += F"{f} : {len(self.visio.cameras[self.numCam].dicoCouleur[f])} mesures\n"
        self.visio.setText1(txt)        
    
    def parametrer(self):
        pmFocused = None
        self.visio.refresh_mini_cube()
        aff_confiance = False
        ...
        while True:
            ...
            self.visio.refresh(self, aff_confiance)
            ...
            if isinstance(event, Click):
                ...
                elif self.enable_modif and event.type == CKCUBE and (event.contenu - 4) % 9 != 0:
                    if pmFocused is not None:
                        pmFocused.resetFocus()
                ...
                elif self.enable_modif and event.type == CKPOSITION:
    ...
    def sauvergardeJSON(self):
        structJson = {"cameras": {}, "robot": {"tempsPas": self.robot.tempsPas, "tempsPause": self.robot.tempsPause, "niveauLed": self.robot.niveauLed}}
        for cam in self.visio.cameras:
            dicoCam = {"PM": [], "recadrage": (cam.y1cut, cam.y2cut, cam.x2cut), 'voisins': cam.dicoCouleur}
            for pm in cam:
    ...
    def recuperationJSON(self):
        ...
        for i in range(self.n):
            ...
            self.visio.cameras[i].dicoCouleur = structJson["cameras"]['ABCD'[i]]["voisins"]
            # print(f"PM de la caméra {i} :")
            ...
            # print(f"mesures de la caméra {i} :")
#             for f in 'FRUBLD':
#                 print(f"{f} -> ", self.visio.cameras[i].dicoCouleur[f])
            # print()
...
class IHMvisio (object):
    def __init__(self, cube):
        ...
        self.bp_mvt = [Bouton(WBP3, HBP3, txt) for txt in TEXTBP3]
        ...
        self.cube = cube
        self.miniCube = self.cube # deepcopy(self.cube)
    ...
    def refresh_mini_cube(self):
        self.miniCube.str2cube(repr(self.cube))

    def appendCamera(self, cam):
        self.cameras.append(cam)
        
    def reconnaitre(self, cube, numCam=None):
        if numCam is not None:
            assert isinstance(numCam, int)
            self.cameras[numCam].reconnaitre(cube)
        else:
            for cam in self.cameras:
                cam.reconnaitre(cube)

    def refresh(self, reco, aff_confiance):
        ...
        reco : Reconnaissance
        """
        image = self.cameras[reco.numCam].getImgIHM()
        
        self.fenetre.blit(image, (0, 0))
        
        for i in range(4):
            self.bp[i].relacher()
        self.bp[reco.numCam].presser()
        
        
        if reco.enable_modif:
            for i in reco.bp_not_enable:
                self.bp[i].enable()
            self.bp[reco.bp_verouillage].relacher()
        else:
            for i in reco.bp_not_enable:
                self.bp[i].disable()
            self.bp[reco.bp_verouillage].presser()
        ...
        for pm in self.cameras[reco.numCam]:
            self.fenetre.blit(pm.image, pm.rect)
        
        self.iMiniCube.refresh(self.cameras[reco.numCam], aff_confiance)
...
#         cube = self.imgTest[self.cmpt][1]
        ...
        for pm in self._pointsMesure:
            if cubeTxt[pm.num] in 'FRUBLD':
                self.dicoCouleur[cubeTxt[pm.num]].append(pm.getColorAverage(im))
        #print(self.dicoCouleur)
    ...
    def reconnaitre(self, cube):
        ...
        for pm in self._pointsMesure:
            ...
            couleur, confiance = self.kVoisins(c)
            cube.setPiece(pm.num, couleur, confiance)
...
#         cube.str2cube("".join(cubeLst))
    ...
    def kVoisins(self, c):
        ...
        for k in 'FRUBLD':
            ...
            if len(self.dicoCouleur[k]) != 0:
                d = 0
                for c2 in self.dicoCouleur[k]:
                    d += ((c[0] - c2[0]) ** 2 + (c[1] - c2[1]) ** 2 + (c[2] - c2[2]) ** 2) ** 0.5
                dicoDist[k] = d / len(self.dicoCouleur[k])

        #print(dicoDist)
        mini = sorted(dicoDist.values())
        for k in dicoDist:
            if dicoDist[k] == mini[0]:
                # confiance sur l'écart relatif entre le mini et le suivant, tend vers 100%
                conf_1 = 100 * (1 - math.exp((mini[0] - mini[1]) / mini[0] / 2))
                # confiance sur la valeur absolue de mini, 10 -> 100%, puis tends vers 0% quand la valeur augmente
                conf_2 = 100 * math.exp(-(mini[0] - 10) / 100) if mini[0] >= 10 else 100
                # moyenne des deux
                confiance = round((conf_1 + conf_2) / 2)
                return k, confiance
